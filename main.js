...
client.on(Events.InteractionCreate, async (interaction) => {
  ...
  // WITH PERMISSIONS
  checkPermissions(con, smCommand.data.permissions, interaction.user.id)
    .then((result) => {
      if (result == true) {
        try {
          ...
          interaction.reply({
            content:
              "There was an error while executing this button's command!",
            ephemeral: true,
          });
          return;
        }
...
client.on(Events.InteractionCreate, (interaction) => {
  ...
  // permission check
  checkPermissions(con, btnCommand.data.permissions, interaction.user.id)
    .then((result) => {
      if (result == true) {
        try {
